...
def canonical_compound(name: str) -> str:
    ...
    - מוציא את כל הספרות לפי סדר הופעה -> "1,2,4".
    - מוחק ספרות וסימני . , - / מהבסיס, ומוחק גם רווחים:
      Propylbenzene == Propyl benzene; Trimethylbenzene == Trimethyl benzene.
    - cis-1.2-Dichloroethene  ↔  1,2-cis-Dichloroethylene
      -> שניהם "1,2 cisdichloroethen".
    """
    s = norm(name)
    ...
    base_no_nums = re.sub(r"\s+", "", base_no_nums)  # remove all spaces
